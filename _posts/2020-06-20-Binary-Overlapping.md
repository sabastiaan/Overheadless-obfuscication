...
The second interesting that happened is that we suddenly obtained a totally new instruction that we have not seen before by starting execution/parsing one byte later, while still using the same original byte sequence!
...
All of these techniques require extensive knowledge of the program to a level typically not available to an assembler. 
